    ...
    <main>
      ...
      <section className="representative-hero section-shell">
        ...
        <div className="representative-hero-copy">
          ...
          <ul className="representative-facts">{representative.careerHighlights.filter((item) => item.status === "verified").map((item) => <li key={item.value}><BadgeCheck />{item.value}</li>)}</ul>
        </div>
      </section>
      ...
      <section className="soft-section section-space"><div className="section-shell"><SectionIntro eyebrow="② 10 YEARS IN THE FIELD" title="10년의 교육·공방 운영 경험" description="확인된 공식 제공 내용을 기준으로 경력을 정리했습니다." /><div className="career-timeline">{representative.careerHighlights.filter((item) => item.status === "verified").map((item, index) => <article key={item.value}><span>{String(index + 1).padStart(2, "0")}</span><div><h3>{item.value}</h3></div></article>)}</div></div></section>
